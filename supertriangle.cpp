...
int main() {
  const gsl_matrix_const_view d_view = gsl_matrix_const_view_array(data, 5, 2);
  
  Gnuplot gp;
  // gp << "set yrange[-1:10]\nset xrange[-3:8]\n";
  gp << "plot '-' with circles, '-' with lines, '-' with lines\n";
  ...
  // construct supertriangle

  double st[6];
  gsl_matrix_view supertriangle_view = gsl_matrix_view_array(st, 3, 2);
  make_supertriangle(&d_view.matrix, &supertriangle_view.matrix, 0);
